...
### Representación típica en Java (linked)

```Java
class TreeNode {
    int value;
    TreeNode left;   // hijo izquierdo (left child)
    TreeNode right;  // hijo derecho (right child)
...
public class Traversals {
    ...
    // Preorder (preorden): Node -> Left -> Right
    ...
    // Inorder (inorden): Left -> Node -> Right
    ...
    // Postorder (postorden): Left -> Right -> Node
...
static List<Integer> preorderIterative(Traversals.TreeNode root) {
    List<Integer> result = new ArrayList<>();
    if (root == null) return result;

    Deque<Traversals.TreeNode> stack = new ArrayDeque<>();
    stack.push(root);

    while (!stack.isEmpty()) {
        Traversals.TreeNode node = stack.pop();
        result.add(node.value);

        // OJO: primero right, luego left, para que left salga antes (LIFO)
...
static List<Integer> inorderIterative(Traversals.TreeNode root) {
    List<Integer> result = new ArrayList<>();
    Deque<Traversals.TreeNode> stack = new ArrayDeque<>();
    Traversals.TreeNode current = root;

    while (current != null || !stack.isEmpty()) {
        // 1) bajar por la izquierda (left spine)
        while (current != null) {
            stack.push(current);
...
En curso DAM normalmente basta con entenderlo; si te lo piden, la versión con 2 stacks es clara:

```Java
import java.util.*;

static List<Integer> postorderTwoStacks(Traversals.TreeNode root) {
    List<Integer> result = new ArrayList<>();
    if (root == null) return result;

    Deque<Traversals.TreeNode> s1 = new ArrayDeque<>();
    Deque<Traversals.TreeNode> s2 = new ArrayDeque<>();
    s1.push(root);

    while (!s1.isEmpty()) {
        Traversals.TreeNode node = s1.pop();
        s2.push(node);
...
static List<Integer> levelOrder(Traversals.TreeNode root) {
    List<Integer> result = new ArrayList<>();
    if (root == null) return result;

    Deque<Traversals.TreeNode> queue = new ArrayDeque<>();
    queue.add(root);

    while (!queue.isEmpty()) {
        Traversals.TreeNode node = queue.remove();
        result.add(node.value);
...
### 4.4.5 Implementación Java (search + insert + delete)
...
public class BSTree {
    ...
    // Insert (inserción) - sin duplicados
    ...
    private Node insertRec(Node node, int key) {
        ...
        // si key == node.key: no insertamos (sin duplicados)
        return node;
    }
    ...
    private Node deleteRec(Node node, int key) {
        ...
        if (key < node.key) {
            ...
            // Caso C: 2 hijos
            // Elegimos sucesor (inorder successor): mínimo del subárbol derecho
...
### 4.6.5 Implementación Java (min-heap básico)
...
public class MinHeap {
    ...
    // Peek min (ver mínimo) => O(1)
    public int peek() {
        if (heap.isEmpty()) throw new NoSuchElementException("Heap vacío (empty heap)");
        return heap.get(0);
    }

    // Insert => O(log n)
    public void insert(int value) {
        heap.add(value);               // lo metemos al final
        siftUp(heap.size() - 1);       // restauramos heap property subiendo
    }
    ...
    private void siftUp(int i) {
        while (i > 0) {
            int parent = (i - 1) / 2;
            if (heap.get(parent) <= heap.get(i)) break; // ya cumple (min-heap)
    ...
    private void siftDown(int i) {
        ...
        while (true) {
            ...
            if (smallest == i) break; // ya está bien
